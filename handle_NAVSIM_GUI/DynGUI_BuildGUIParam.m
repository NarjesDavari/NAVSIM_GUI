function handles = DynGUI_BuildGUIParam(Param)
...
% set([fig,handles.htxt_title ,handles.htxt_units ,handles.h,handles.pushbutton_OK,handles.pushbutton_Cancel,handles.text_choix,handles.popupmenu_choix,handles.pushbutton_reset ],'Units','normalized');

% handles = guidata(handles.fig,handles);
%$$$$$$$$$$$$$$$$$$$
uiwait(handles.fig);
%$$$$$$$$$$$$$$$$$$$
if length(handles.output)==1
    handles.output{2}= 'Close';
end
try
    delete(handles.fig);
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function menu_choix_callback(hObject,eventdata)
        ...
        handles.Iold = I;
        for J=1:length(handles.h)
            if ~strcmp(get(handles.h(J),'Style'),'checkbox')
                delete(handles.htxt_title(J));
                delete(handles.htxt_units(J));
            end
            delete(handles.h(J));
        end
        ...
        if strcmp(Param(I).style,'checkbox')
            ...
            if strcmp(Param(I).style,'edit')
                ...
                if ischar(Param(I).val)
                    h(I) = uicontrol('Parent',fig,'Style','edit','string',Param(I).val,'Position', [2*spacing+width posfig(4)-spacing-start-(I-1)*(spacing+height) width height],'TooltipString',Param(I).tooltipstring);
                else
                    h(I) = uicontrol('Parent',fig,'Style','edit','string',num2str(Param(I).val),'Position', [2*spacing+width posfig(4)-spacing-start-(I-1)*(spacing+height) width height],'TooltipString',Param(I).tooltipstring);
                end 
