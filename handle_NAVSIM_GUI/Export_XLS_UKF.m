function Export_XLS_UKF(Simulation)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for I=1:length(index)
    switch I
        ...
        case 2
            ...
            if isfield(Simulation.Output.User_Def_Sim,'INS_UKF')
                index(I) = 1;
            end
        case 3
            % Designed Path
        ...
        case 5
            ...
            if isfield(Simulation.Output.PostProc_Real,'INS_UKF')
                index(I) = 1;
            end
        case 6
...
switch I
    ...
    case 2
        ...
        if isfield(Simulation.Input.User_Def_Sim,'Path') &&...
           ...
           isfield(Simulation.Output.User_Def_Sim,'INS_UKF')
           
           name = 'System State'; 
           Tab  = NAVSIM_GenTab_SystemStateUDPS(Simulation,'UKF');
        end  
    case 3
        % Designed Path
        if isfield(Simulation.Input.User_Def_Sim,'Path')
    ...
    case 4
        ...
        if isfield(Simulation.Output.User_Def_Sim,'INS_UKF')
            name = 'Error Reports';
            Tab  = NAVSIM_GenTab_ErrorUDPS(Simulation,'UKF');
        end  
    case 5
        %System state in Processing of Real Measurments
        if isfield(Simulation.Input.PostProc_Real,'Measurements') &&...     
           isfield(Simulation.Output.PostProc_Real,'INS_UKF')
           
           name = 'System State'; 
           Tab  = NAVSIM_GenTab_SystemStatePRM(Simulation,'UKF');
        end  
    case 6
        %Error Reports in Processing of Real Measurments
        if isfield(Simulation.Output.PostProc_Real,'INS_UKF')
            name = 'Error Reports';
            Tab  = NAVSIM_GenTab_ErrorPRM(Simulation,'UKF');
        end
    case 7
        % Simulated signals
        if isfield(Simulation.Output.User_Def_Sim,'Noise')           
